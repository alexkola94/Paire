...
import {
  DollarSign, Users, Target, Shield, Check, ArrowRight,
  PieChart, Bell, ShoppingCart, MapPin,
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={styles.hero}>
          ...
          <View style={[styles.badge, { backgroundColor: theme.colors.surfaceSecondary, borderColor: theme.colors.glassBorder }]}>
            ...
            <Text style={[styles.badgeText, { color: theme.colors.textSecondary }]}>{t('landing.hero.badge')}</Text>
          </View>
          <Text style={[styles.heroTitle, { color: theme.colors.text }]}>{t('landing.hero.title')}</Text>
          <Text style={[styles.heroSubtitle, { color: theme.colors.textSecondary }]}>{t('landing.hero.subtitle')}</Text>
          <View style={styles.heroActions}>
            <Button title={t('landing.hero.getStarted')} onPress={goToSignUp} variant="primary" rightIcon={ArrowRight} style={styles.heroPrimaryBtn} />
            <Button title={t('landing.hero.login')} onPress={goToLogin} variant="outline" style={styles.heroSecondaryBtn} />
          </View>
          <View style={styles.trust}>
            {[1, 2, 3].map((i) => (
              <View key={i} style={styles.trustItem}>
                <Check size={16} color={theme.colors.primary} />
                <Text style={[styles.trustText, { color: theme.colors.textSecondary }]}>{t(`landing.hero.trust${i}`)}</Text>
              </View>
            ))}
          </View>
        </View>
        ...
        <View style={[styles.ctaBlock, { backgroundColor: theme.colors.surfaceSecondary }]}>
          ...
          <Button title={t('landing.cta.button')} onPress={goToSignUp} variant="primary" rightIcon={ArrowRight} style={styles.ctaButton} />
        </View>
...
const styles = StyleSheet.create({
  ...
  heroLogoWrapper: { alignItems: 'center', marginBottom: spacing.lg },
  badge: { flexDirection: 'row', alignItems: 'center', alignSelf: 'flex-start', paddingVertical: spacing.xs, paddingHorizontal: spacing.sm, borderRadius: borderRadius.full, borderWidth: 1, gap: spacing.xs },
  ...
  featureCard: { flexDirection: 'row', alignItems: 'center', padding: spacing.md, borderRadius: borderRadius.md, marginBottom: spacing.sm, gap: spacing.sm },
  featureTitle: { ...typography.label, flex: 1 },
  stepCard: { padding: spacing.md, borderRadius: borderRadius.md, marginBottom: spacing.sm },
  ...
  statCard: { flex: 1, minWidth: '28%', padding: spacing.md, borderRadius: borderRadius.md, alignItems: 'center' },
  ...
  ctaBlock: { marginHorizontal: spacing.lg, padding: spacing.lg, borderRadius: borderRadius.lg, alignItems: 'center', marginBottom: spacing.xl },
