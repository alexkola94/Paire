 ...
 * Same contract as frontend buildTransportLegs for consistency.
 *
 * @typedef {Object} Leg
 ...
 * @param {string|null} params.homeToFirstTransport - Mode for home → first city leg (mobile-specific)
 ...
 * @returns {Leg[]}
 */
export function buildTransportLegs({
  orderedCities,
  homeLocation,
  homeCityName,
  returnTransportMode,
  ...
  if (hasHomeContext && firstCity) {
    legs.push({
      ...
      endDate: null,
      transportMode: homeToFirstTransport || firstCity.transportMode || null,
      isContext: true,
    });
  }
