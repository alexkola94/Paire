...
-- This enables:
-- - Creating/editing transactions (expenses & income)
-- - Creating/editing loans
-- - Managing budgets
...
USING (
  EXISTS (
    ...
    WHERE partnerships.status = 'active'
    AND (
      (partnerships.user1_id::text = auth.uid()::text AND partnerships.user2_id = budgets.user_id)
      OR
      (partnerships.user2_id::text = auth.uid()::text AND partnerships.user1_id = budgets.user_id)
    )
  )
);
...
BEGIN
  ...
  RAISE NOTICE '';
  RAISE NOTICE '🎉 Your app should now work completely!';
  RAISE NOTICE '';
  RAISE NOTICE '👉 Next steps:';
  RAISE NOTICE '   1. Refresh your frontend app';
  RAISE NOTICE '   2. Try adding an expense or income';
  RAISE NOTICE '   3. Check the Analytics page';
  RAISE NOTICE '';
END $$;
