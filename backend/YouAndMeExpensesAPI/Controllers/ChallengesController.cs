...
namespace YouAndMeExpensesAPI.Controllers
{
    ...
    public class ChallengesController : BaseApiController
    {
        ...
        public async Task<IActionResult> GetAvailableChallenges()
        {
            try
            {
                var challenges = await _challengeService.GetAvailableChallengesAsync();
                return Ok(challenges);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error fetching available challenges");
        ...
        public async Task<IActionResult> GetUserChallenges([FromQuery] string? status = null)
        {
            ...
            try
            {
                var challenges = await _challengeService.GetUserChallengesAsync(userId.ToString(), status);
                return Ok(challenges);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error getting challenges for user {UserId}", userId);
                return StatusCode(500, new { error = "Failed to retrieve user challenges" });
            }
        }
        ...
        public async Task<IActionResult> JoinChallenge(Guid challengeId)
        {
            ...
            try
            {
                var userChallenge = await _challengeService.JoinChallengeAsync(userId.ToString(), challengeId);
            ...
            {
                _logger.LogError(ex, "Error joining challenge {ChallengeId} for user {UserId}", challengeId, userId);
                return StatusCode(500, new { error = "Failed to join challenge" });
            }
        }
        ...
        public async Task<IActionResult> UpdateProgress(Guid userChallengeId, [FromBody] UpdateProgressRequest request)
        {
            ...
            try
            {
                ...
                if (result == null)
                    return NotFound(new { error = "Active challenge not found" });
                return Ok(result);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error updating challenge progress for user {UserId}", userId);
        ...
        public async Task<IActionResult> ClaimReward(Guid userChallengeId)
        {
            ...
            try
            {
                ...
                if (result == null)
                    return NotFound(new { error = "Completed challenge not found or reward already claimed" });
                return Ok(result);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error claiming reward for user {UserId}", userId);
                return StatusCode(500, new { error = "Failed to claim reward" });
            }
        }
        ...
        public async Task<IActionResult> SeedChallenges()
        {
            try
            {
                await _challengeService.SeedDefaultChallengesAsync();
                return Ok(new { message = "Challenges seeded successfully" });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error seeding challenges");
                return StatusCode(500, new { error = "Failed to seed challenges" });
            }
        }
    }
