...
namespace YouAndMeExpensesAPI.Controllers
{
    ...
    public class WeeklyRecapController : BaseApiController
    {
        ...
        /// <summary>
        /// Get the latest weekly recap for the authenticated user.
        /// </summary>
        [HttpGet]
        ...
        /// <summary>
        /// Get recap history for the authenticated user.
        /// </summary>
        /// <param name="count">Number of recaps to return (default: 4)</param>
        [HttpGet("history")]
        ...
        /// <summary>
        /// Manually trigger recap generation for the authenticated user.
        /// </summary>
        [HttpPost("generate")]
        [ProducesResponseType(typeof(WeeklyRecap), 200)]
        public async Task<IActionResult> Generate()
        {
            ...
            try
            {
                var recap = await _weeklyRecapService.GenerateRecapAsync(userId.ToString());
                return Ok(recap);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error generating recap for user {UserId}", userId);
                return StatusCode(500, new { error = "Failed to generate recap" });
            }
        }
    }
}
