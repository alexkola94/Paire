...
public class TransactionsService : ITransactionsService
{
    ...
    private async Task<List<string>> GetUserAndPartnerIdsAsync(Guid userId)
    {
        try
        {
            return await _partnershipResolver.GetHouseholdUserIdsAsync(userId.ToString());
        }
        catch (Exception ex)
        {
    ...
    public async Task<TransactionsPageDto> GetTransactionsAsync(
        Guid userId, string? type, DateTime? startDate, DateTime? endDate,
        int? page, int? pageSize, string? search)
    {
        ...
        if (!string.IsNullOrWhiteSpace(search))
        {
            ...
            if (trimmedSearch.Length > 200) trimmedSearch = trimmedSearch.Substring(0, 200);
        ...
        if (startUtc.HasValue) query = query.Where(t => t.Date >= startUtc.Value);
        if (endUtc.HasValue) query = query.Where(t => t.Date <= endUtc.Value);
        ...
        {
            if (effectivePage.Value < 1) effectivePage = 1;
            if (!effectivePageSize.HasValue || effectivePageSize.Value <= 0) effectivePageSize = 50;
            const int MaxPageSize = 200;
            if (effectivePageSize.Value > MaxPageSize) effectivePageSize = MaxPageSize;
        }
        ...
        var result = new TransactionsPageDto { Items = enriched };
    ...
    public async Task<CreateTransactionResponseDto> CreateTransactionAsync(Guid userId, CreateTransactionRequest request)
    {
        ...
        if (transaction.Type.ToLower() == "expense")
        {
            budgetAlerts = await _budgetService.UpdateSpentAmountAsync(
                userId.ToString(), transaction.Category, transaction.Amount, transaction.Date);
        }
        ...
        var transactionDto = ToDto(transaction, userProfile);
    ...
    public async Task<Transaction?> UpdateTransactionAsync(Guid userId, Guid id, Transaction transaction)
    {
        ...
        existing.Date = transaction.Date.Kind == DateTimeKind.Utc ? transaction.Date : transaction.Date.ToUniversalTime();
        ...
        if (oldType.ToLower() == "expense")
            await _budgetService.UpdateSpentAmountAsync(userId.ToString(), oldCategory, -oldAmount, oldDate);

        if (existing.Type.ToLower() == "expense")
            await _budgetService.UpdateSpentAmountAsync(userId.ToString(), existing.Category, existing.Amount, existing.Date);
    ...
    public async Task<bool> DeleteTransactionAsync(Guid userId, Guid id)
    {
        ...
        if (transaction.Type.ToLower() == "expense")
            await _budgetService.UpdateSpentAmountAsync(userId.ToString(), transaction.Category, -transaction.Amount, transaction.Date);
    ...
    public async Task<ReceiptUploadResultDto> UploadReceiptAsync(Guid userId, IFormFile file)
    {
        ...
        return new ReceiptUploadResultDto { Url = receiptUrl, Path = fileName };
    }

    public async Task<IReadOnlyList<TransactionWithProfileDto>> GetTransactionsWithReceiptsAsync(
        Guid userId, string? category, string? search)
    {
        var allUserIds = await GetUserAndPartnerIdsAsync(userId);
        var query = _dbContext.Transactions
            .AsNoTracking()
