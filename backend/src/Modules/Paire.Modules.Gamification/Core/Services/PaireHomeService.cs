...
public class PaireHomeService : IPaireHomeService
{
    ...
    public PaireHomeService(GamificationDbContext context, IPartnershipResolver partnershipResolver, ILogger<PaireHomeService> logger)
    {
        _context = context;
    ...
    {
        var home = await _context.PaireHomes.FirstOrDefaultAsync(h => h.UserId == userId);
    ...
    public async Task<PaireHome> ProcessExpenseAsync(string userId, string category, decimal amount)
    {
        ...
        var room = MapCategoryToRoom(category) ?? "living_room";
        var pointsToAdd = (int)Math.Floor(amount);
        if (pointsToAdd <= 0) return home;
    ...
    {
        if (!FurnitureCatalog.RoomFurniture.TryGetValue(room, out var furnitureList)) return;
    ...
    public async Task<object> GetFurnitureAsync(string userId)
    {
        var unlocked = await _context.HomeFurniture
            ...
            .OrderBy(f => f.RoomName).ThenBy(f => f.UnlockedAt)
            .ToListAsync();
        ...
        foreach (var (room, catalog) in FurnitureCatalog.RoomFurniture)
        {
            ...
            result[room] = new
            {
                catalog = catalog.Select((code, index) => new { code, unlocked = roomUnlocked.Contains(code), equipped = roomEquipped.Contains(code), unlockPoints = FurnitureCatalog.GetUnlockPointsThreshold(index) }),
    ...
    public async Task<object> EquipFurnitureAsync(string userId, string room, string furnitureCode, bool equip)
    {
        ...
        if (!isOwned) throw new InvalidOperationException($"Furniture {furnitureCode} is not unlocked");

        var home = await GetOrCreateHomeAsync(userId);
        var decorations = DeserializeDecorations(home.Decorations);
        if (!decorations.ContainsKey(room)) decorations[room] = new List<string>();

        if (equip && !decorations[room].Contains(furnitureCode)) decorations[room].Add(furnitureCode);
        else if (!equip) decorations[room].Remove(furnitureCode);
    ...
    public async Task<object> GetCoupleHomeAsync(string userId)
    {
        ...
        if (partnerId == null) return new { hasPartner = false };
        ...
        foreach (var room in RoomCategoryMapping.Keys)
            combinedPoints[room] = myRoomPoints.GetValueOrDefault(room, 0) + partnerRoomPoints.GetValueOrDefault(room, 0);
    ...
    public async Task<object> GetRoomsAsync(string userId)
    {
        ...
        foreach (var (roomKey, _) in RoomCategoryMapping)
        {
            ...
            if (isUnlocked && level == 0) level = 1;
            var pointsToNextLevel = GetPointsToNextLevel(points, level);
            rooms.Add(new { name = roomKey, level, points, isUnlocked, pointsToNextLevel });
        }
        return rooms;
    }
    ...
    public async Task<PaireHome> UpgradeRoomAsync(string userId, string room)
    {
        ...
        if (!unlockedRooms.Contains(roomKey)) unlockedRooms.Add(roomKey);
    ...
    {
        if (points < UnlockThreshold) return 0;
        var level = 1;
        foreach (var threshold in LevelThresholds)
        {
            if (points >= threshold) level++;
            else break;
        }
        return level;
    }
    ...
    {
        var totalLevel = unlockedRooms.Sum(room => roomLevels.GetValueOrDefault(room, 1));
    ...
    private static int? GetPointsToNextLevel(int points, int level)
    {
        ...
        if (level == 0) return UnlockThreshold - points;
    ...
    private static Dictionary<string, List<string>> DeserializeDecorations(string json) =>
        JsonSerializer.Deserialize<Dictionary<string, List<string>>>(json) ?? new Dictionary<string, List<string>>();
}
