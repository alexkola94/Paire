/**
 * Skyscanner Flights Live Prices API – create then poll.
 * Partner: https://developers.skyscanner.net or RapidAPI.
 * Env: VITE_SKYSCANNER_API_KEY (Partner) or VITE_RAPIDAPI_KEY + VITE_RAPIDAPI_SKYSCANNER_HOST for RapidAPI.
 */
...
const RAPIDAPI_SKY_HOST = import.meta.env.VITE_RAPIDAPI_SKYSCANNER_HOST || ''

const useRapidAPI = Boolean(RAPIDAPI_KEY && RAPIDAPI_SKY_HOST)

/** City/place name to IATA code for Skyscanner (lowercase slug -> code). */
const CITY_TO_IATA = {
...
/**
 * Resolve origin/destination (city name or IATA) to IATA code for Skyscanner.
 */
...
export const createSearch = async (params) => {
  ...
  if (!originIata || !destinationIata || !outboundDate) {
    throw new Error('Missing origin, destination or outbound date')
  }
  ...
  const url = useRapidAPI
    ? `https://${RAPIDAPI_SKY_HOST}/flights/live/search/create`
    : `${PARTNER_BASE}/create`
  const headers = {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Skyscanner create: ${res.status} ${text}`)
  }
...
/**
 * Poll for results. Returns { status, itineraries }.
...
export const pollResults = async (sessionToken) => {
  ...
  const url = useRapidAPI
    ? `https://${RAPIDAPI_SKY_HOST}/flights/live/search/poll/${sessionToken}`
    : `${PARTNER_BASE}/poll/${sessionToken}`
...
/**
 * Run create then poll until complete or timeout. Returns normalized itineraries.
 * @param {{ originIata: string, destinationIata: string, outboundDate: string, returnDate?: string, adults?: number }} params
 */
export const searchFlights = async (params) => {
  ...
  while (Date.now() - start < POLL_TIMEOUT_MS) {
    ...
    if (status === 'RESULT_STATUS_COMPLETE') {
      ...
      const segments = results.segments || results.Segments || {}
